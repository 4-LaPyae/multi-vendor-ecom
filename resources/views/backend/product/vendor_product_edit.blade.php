    ...
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body p-4">
                ...
                <form id="myForm" method="post" action="{{ route('vendorproducts.update',$products->id) }}">
                    @csrf
                    @method('put')
                ...
                <table class="table mb-0 table-striped">
                    ...
                    <tbody>
                        ...
                        <form method="post" action="{{ route('vendor.update.product.multiimage') }}" enctype="multipart/form-data">
                            @csrf
                            @foreach ($products->images as $key => $img)
                                <tr>
                                    <th scope="row">{{ $key + 1 }}</th>
                                    <td> <img id="changemultiimage" src="{{ asset($img->photo_name) }}" style="width:70; height: 40px;"> </td>
                                    <td> <input type="file" id="multiimage" class="form-group" name="multi_img[{{ $img->id }}]">
                                    </td>
                                    <td>
                                        <input type="submit" class="btn btn-primary px-4" value="Update Image " />
                                        <a href="{{ route('vendor.product.multiimg.delete',$img->id) }}" class="btn btn-danger" id="deleteinatag"> Delete </a>
                                    </td>
                                </tr>
                            @endforeach
